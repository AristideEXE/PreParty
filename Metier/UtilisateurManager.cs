...
namespace Metier
{
    public class UtilisateurManager
    {
        ...
        public static List<Notification> GetNotificationsPasLus(int idUtilisateur)
        {
            List<Notification> notifications = new List<Notification>();
            try
            {
                string query = "SELECT idNotification, notif, lu, redirection FROM notification WHERE idUtilisateur = @idUtilisateur AND lu = 0 ORDER BY idNotification DESC";
                using (MySqlConnection conn = Connexion.GetConnection())
                {
                    conn.Open();
                    MySqlCommand cmd = conn.CreateCommand();
